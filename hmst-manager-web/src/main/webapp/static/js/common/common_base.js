...
var pageSize = 10;
//列表展示
function getTableList(url, data, showPage, successCallback, failCallback,search) {
    var pageData = {
            pageNo: search==true?1:current,
            pageSize: pageSize
    }
    if (showPage) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.ret) {
                ...
                if (showPage) {
                    createPage(url, result, data, successCallback);
                }
            } else {
                if (failCallback) {
                    failCallback(result);
...
function createMustache(listJson, result, listTemplate, ele) {
    ...
    var resultJson = {};
    resultJson[firstKey] = result.data.list;
...
function createPage(url, result, data, successCallback) {
    pageNum = parseInt(parseInt(result.data.total) / pageSize) + 1;
